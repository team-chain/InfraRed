...
## 역할 A 구현 현황 (feat/a-infra)

### Docker Agent (`agent/`)

| 항목 | 상태 | 비고 |
|------|------|------|
| auth.log tailing | ✅ | `tailer.py` — polling 방식 (2초), watchdog 의존성 추가됨 |
| log rotation 대응 | ✅ | inode 변경 감지 → 새 파일 자동 재연결 |
| offset SQLite 저장 | ✅ | `offset_store.py` — 재시작 시 이어서 전송 |
| 네트워크 단절 SQLite 버퍼링 | ✅ | `buffer_store.py` — 전송 실패 시 WAL 모드 SQLite에 적재 |
| 재연결 후 순서대로 재전송 | ✅ | `client.flush_buffer()` — FIFO 순서, 5xx/네트워크 에러만 재시도 |
| 민감정보 마스킹 | ✅ | `masking.py` — password/token/api_key/session/Authorization/Cookie/이메일/전화번호 |
| Heartbeat 30초 | ✅ | `main.py` — `pending_buffered_events` 수 함께 전송 |

### Ingestion API (`backend/app/ingestion/`)

| 검증 순서 | 항목 | HTTP 응답 | 상태 |
|-----------|------|-----------|------|
| 1 | JWT 유효성 검증 | 401 | ✅ |
| 2 | tenant_id 클레임 일치 | 403 | ✅ |
| 3 | agent_id 클레임 일치 | 403 | ✅ |
| 4 | payload 크기 64KB 이하 | 413 | ✅ |
| 5 | Content-Type: application/json | 415 | ✅ |
| 6 | timestamp 필드 존재 | 422 | ✅ |
| 7 | event_id 필드 존재 | 422 | ✅ |
| 8 | 5분 이내 이벤트 정상 처리 | 202 | ✅ |
| 9 | 5분~24시간 late_event=true | 202 | ✅ |
| 10 | 24시간 초과 거부 | 410 | ✅ |
