...
async def create_user(
    *,
    ...
    """Insert a new user and return the created row, or None if email already exists."""
    async with get_session() as session:
        # Check tenant exists
        ...
        dup_check = await session.execute(
            text("SELECT 1 FROM users WHERE tenant_id = :tenant_id AND email = :email"),
        ...
        result = await session.execute(
            ...
            {"tenant_id": tenant_id, "email": email, "password": password, "role": role},
        )
