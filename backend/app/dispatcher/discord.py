...
──────────────────────
Phase 1 — send_discord_phase1_alert()
  탐지 즉시 전송 (LLM 분석 전). 빠른 인지를 위해 경량 embed 사용.

Phase 2 — send_discord_embed()
  LLM 분석 완료 후 전송. 문제 / 권장 조치 구조의 rich embed.

Demo Debounce
─────────────
/demo 엔드포인트는 같은 webhook으로 30초 이내 중복 전송을 차단한다.
check_demo_debounce(url) → True면 이미 최근에 보냈음 (전송 생략).
"""
...
# Demo webhook debounce — in-memory, per webhook URL, 30초 쿨다운
DEMO_DEBOUNCE_SECONDS: int = 30
_demo_last_sent: dict[str, float] = {}


def check_demo_debounce(webhook_url: str) -> bool:
    """True이면 debounce 중 → 전송 생략. False면 전송 허용 후 타임스탬프 기록."""
    now = time.monotonic()
    last = _demo_last_sent.get(webhook_url)
    if last is not None and (now - last) < DEMO_DEBOUNCE_SECONDS:
        return True
    _demo_last_sent[webhook_url] = now
    return False
...
async def send_discord_phase1_alert(
    *,
    incident_id: str,
    tenant_id: str,
    severity: str,
    ...
    """Phase 1 — 탐지 즉시 전송 (LLM 분석 前).

    LLM 분석 결과 없이 탐지 기본 정보만으로 빠르게 채널에 알린다.
    Phase 2(send_discord_embed)가 뒤따라 상세 내용을 전송한다.
    """
    ...
    fields = [
        {"name": "심각도", "value": severity.upper(), "inline": True},
        {"name": "MITRE 전술", "value": mitre_tactic or "-", "inline": True},
        {"name": "Kill Chain", "value": kill_chain_stage or "-", "inline": True},
    ]
    if source_ip:
        fields.append({"name": "Source IP", "value": f"`{source_ip}`", "inline": True})

    embed = {
        "title": f"{severity_emoji} [{severity.upper()}] 인시던트 탐지 — {incident_id}",
        "description": "⏳ AI 분석이 진행 중입니다. 분석 완료 후 상세 내용이 전송됩니다.",
        "color": color,
        "fields": fields,
        "footer": {"text": f"InfraRed SOC — {tenant_id}  |  Phase 1 / 2"},
    }
...
async def send_discord_embed(
    *,
    ...
    """Phase 2 — LLM 분석 완료 후 Rich embed 전송 (문제 / 권장 조치 구조).

    Phase 1(send_discord_phase1_alert)이 선행 전송된 후 이 함수가 호출된다.
