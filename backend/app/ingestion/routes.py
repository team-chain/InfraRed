...
async def ingest_event(
    ...
    # ── 검증 5: Content-Type: application/json 확인 ──────────────────
    content_type = request.headers.get("content-type", "")
    if "application/json" not in content_type:
        raise HTTPException(
            status_code=status.HTTP_415_UNSUPPORTED_MEDIA_TYPE,
            detail="content_type_must_be_application_json",
        )

    # ── 검증 4: payload 크기 64KB 이하 ───────────────────────────────
    body = await request.body()
    if len(body) > settings.payload_max_bytes:
        raise HTTPException(status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE)

    # ── 검증 6/7: timestamp / event_id 필드 + 스키마 파싱 ───────────
    try:
    ...
    if age.total_seconds() > settings.late_event_max_seconds:
        # 24시간 초과 → 410 Gone (설계서 3.1 검증 순서 10)
        await _write_deadletter(
            envelope.tenant_id,
            "event_too_old",
        ...
        raise HTTPException(status_code=status.HTTP_410_GONE, detail="event_too_old")
